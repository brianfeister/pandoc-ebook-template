...
\definecolor{CalloutInfoColor}{HTML}{C8A348}
...
% table header (and generally row-wise, if used correctly) styling
\newcommand\setrowfont[1]{\noalign{\gdef\rowfont{#1}}}
\gdef\rowfont{}
\def\endhead{\setrowfont{}}
...
% holds header styles
\NewDocumentCommand{\thetableheaderstyle}{}{}
\NewDocumentCommand{\tableheaderstyle}{m}
  {\RenewDocumentCommand{\thetableheaderstyle}{}{#1}}
...
\NewDocumentCommand{\VariaTableStyleDefault}{}{
  ...
  % If you use \cellcolor then you need \leavevmode before that;
  % that's needed to preserve the correct row height
  \tableheaderstyle{\leavevmode\cellcolor{black}\color{white}\bfseries}
}
...
% content alignment
\newcolumntype{J}{>{\bgroup\rowfont}}
\newcolumntype{L}{>{\bgroup\rowfont\raggedright\arraybackslash}}
\newcolumntype{C}{>{\bgroup\rowfont\centering\arraybackslash}}
\newcolumntype{R}{>{\bgroup\rowfont\raggedleft\arraybackslash}}

% instead of erasing separators with @{} just compensate for them
\newcolumntype{S}[2]{#1{\dimexpr#2\linewidth-2\tabcolsep\relax}}

% cell alignment
\newcolumntype{T}[1]{S{p}{#1}<{\egroup}}
\newcolumntype{M}[1]{S{m}{#1}<{\egroup}}
\newcolumntype{B}[1]{S{b}{#1}<{\egroup}}

\newif\ifnexttablehasheader
\NewDocumentCommand{\NextTableColumns}{sm}{%
\newcolumntype{N}{#2}%
\IfBooleanTF{#1}{\nexttablehasheadertrue}{\nexttablehasheaderfalse}%
}

\newcommand{\maybethetableheader}
  {\ifnexttablehasheader\setrowfont{\thetableheaderstyle}\fi}
...
\NewDocumentCommand{\BreakingTable}{}{
  \renewenvironment{VariaTable}
    {\medskip\begin{xtabular*}{\linewidth}{N}\maybethetableheader}
    {\end{xtabular*}\medskip}
}

\NewDocumentCommand{\FloatingTable}{O{htb}}{
  \renewenvironment{VariaTable}
    {\begin{table}[#1]\TheCaption\begin{xtabular*}{\linewidth}{N}\maybethetableheader}
